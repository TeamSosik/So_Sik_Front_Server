...
const PageButton = ({ handlePageBtnClick, pageData }) => {
  ...
  const getPageButton = () => {
    ...
    if (nowPageSite > 1) {
      result.push(
        <button id={(nowPageSite - 1) * showPages} onClick={handlePageBtnClick}>
          {"<<"}
        </button>
      );
    }
    ...
    if (pageNum > 1) {
      result.push(
        <button id={nowPage - 1} onClick={handlePageBtnClick}>
          {"<"}
        </button>
      );
    }
    ...
    for (let i = 1; i <= showPages; i++) {
      ...
      if ((nowPageSite - 1) * showPages + i === lastPageNum) {
        result.push(
          <button
            id={(nowPageSite - 1) * showPages + i}
            onClick={handlePageBtnClick}
          >
            {(nowPageSite - 1) * showPages + i}
          </button>
        );
        break;
      }
      ...
      if ((nowPageSite - 1) * showPages + i === nowPage) {
        result.push(
          <button className="nowPageBtn" disabled>
            {(nowPageSite - 1) * showPages + i}
          </button>
        );
        continue;
    ...
    if (nowPageSite < lastPageSite) {
      result.push(
        <button id={nowPageSite * showPages + 1} onClick={handlePageBtnClick}>
          {">>"}
        </button>
      );
    }
  ...
  return (
    <div className="pageButtonBox">
      <div className="pageButtonList">{getPageButton()}</div>
    </div>
  );
};
